...
describe("modelo e envio de relatório de aprovações", () => {
  ...
  it("envia somente o PDF gerado ao contato informado com o remetente configurado", async () => {
    ...
    const sent = await sendApprovalHistoryReportEmail({ clientName: "Globo Acabamentos", contactName: "Marina", campaignName: "Lançamento Premium", recipientEmail: "marina@globo.example", criteria: "Responsável: todos", report: { contentBase64: "JVBERi0=", fileName: "historico.pdf", mimeType: "application/pdf", recordCount: 1 } });
    ...
    expect(payload).toMatchObject({ to: ["marina@globo.example"], attachments: [{ filename: "historico.pdf", content: "JVBERi0=" }] });
  });
});
