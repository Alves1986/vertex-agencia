...
import { Bot, BrainCircuit, CheckCircle2, CheckSquare, ChevronRight, Clapperboard, Copy, ExternalLink, FileText, FolderOpen, FolderPlus, ImagePlus, KeyRound, Layers3, Loader2, Pencil, Plus, RefreshCw, Save, ShieldCheck, Sparkles, ToggleLeft, ToggleRight, Trash2, UploadCloud, WandSparkles, XCircle } from "lucide-react";
...
export const providerCatalog: Array<{ value: ProviderKind; label: string; defaultModel: string; description: string; docsUrl?: string; docsLabel?: string }> = [
  { value: "manus", label: "Manus integrado", defaultModel: "gpt-5-mini", description: "Usa o motor integrado, sem colar uma chave externa." },
  { value: "openai", label: "OpenAI", defaultModel: "gpt-5-mini", description: "Para texto, estratégia e ideação do cliente.", docsUrl: "https://developers.openai.com/api/docs", docsLabel: "Documentação da OpenAI" },
...
export function getProviderTestDiagnostic(message?: string | null) {
  ...
  if (normalized.includes("chave") || normalized.includes("401") || normalized.includes("403")) return { title: "A chave não foi aceita", detail: "O provedor recusou a credencial informada. Confirme se a chave está completa, ativa e autorizada para este projeto.", action: "Crie ou selecione uma chave com permissão de API e teste novamente." };
  if (normalized.includes("tempo esperado")) return { title: "O provedor demorou para responder", detail: "A validação excedeu o tempo seguro de espera. A chave não foi salva nem enviada para o navegador.", action: "Aguarde alguns instantes, confira o status do provedor e repita o teste." };
  if (normalized.includes("alcançar")) return { title: "Não foi possível alcançar o provedor", detail: "A VERTEX não conseguiu estabelecer uma conexão segura com a API configurada.", action: "Revise a URL base, o endereço do gateway e eventuais restrições de rede." };
  if (normalized.includes("url") || normalized.includes("modelo") || normalized.includes("permissões") || normalized.includes("confirmou")) return { title: "A configuração precisa de revisão", detail: "O provedor não confirmou a combinação de URL, modelo e permissões informada.", action: "Confira o modelo disponível, a URL base e as permissões da credencial na documentação do provedor." };
  if (normalized.includes("conexão não encontrada")) return { title: "A conexão não está mais disponível", detail: "O registro de conexão não foi localizado para o cliente selecionado.", action: "Feche este card, abra a conexão novamente e repita a validação." };
  return { title: "A validação não foi concluída", detail: "O provedor não confirmou a configuração com segurança. Nenhuma credencial foi salva.", action: "Revise os dados autorizados do cliente e tente novamente." };
}
...
export default function Agency() {
  ...
  const testProviderConnection = trpc.agency.testProviderConnection.useMutation({ onSuccess: (data, variables) => { setProviderTestState("passed"); setProviderTestError(null); setProviderTestFingerprint(`${variables.provider}|${variables.apiBaseUrl ?? ""}|${variables.defaultModel}|${variables.apiKey ?? ""}`); setProviderVerificationToken("verificationToken" in data ? data.verificationToken : ""); toast.success(data.message); }, onError: error => { setProviderTestState("failed"); setProviderTestError(error.message); setProviderTestFingerprint(""); setProviderVerificationToken(""); toast.error("A conexão não foi validada. Revise a orientação exibida no card."); } });
...
function ProviderBrandIcon({ provider }: { provider: ProviderKind }) {
  if (provider === "openai") return <span className="agency-provider-brand agency-provider-brand-openai" aria-hidden="true"><img src="/manus-storage/openai-developers_dc07b702.svg" alt="" /></span>;
  if (provider === "gemini" || provider === "anthropic") {
    const icon = provider === "gemini" ? siGooglegemini : siAnthropic;
    return <span className={`agency-provider-brand agency-provider-brand-${provider}`} aria-hidden="true"><svg viewBox="0 0 24 24"><path d={icon.path} /></svg></span>;
  }
  return <span className="agency-provider-brand agency-provider-brand-generic" aria-hidden="true">{provider === "manus" ? <Bot size={17} /> : <KeyRound size={16} />}</span>;
}

export function ProviderConnectionDialog({ open, onOpenChange, onSubmit, onCancel, provider, setProvider, editing, selectedProvider, saving, testing, testState, testError, testIsCurrent, requiresTest, requiresNewKey, onConfigurationChange, onTest }: { open: boolean; onOpenChange: (open: boolean) => void; onSubmit: (event: FormEvent) => void; onCancel: () => void; provider: ProviderForm; setProvider: (provider: ProviderForm) => void; editing: boolean; selectedProvider: { value: ProviderKind; label: string; defaultModel: string; description: string; docsUrl?: string; docsLabel?: string }; saving: boolean; testing: boolean; testState: "idle" | "passed" | "failed"; testError: string | null; testIsCurrent: boolean; requiresTest: boolean; requiresNewKey: boolean; onConfigurationChange: () => void; onTest: () => void }) {
  const update = (partial: Partial<ProviderForm>, invalidatesValidation = false) => { if (invalidatesValidation) onConfigurationChange(); setProvider({ ...provider, ...partial }); };
  const chooseProvider = (next: ProviderKind) => {
    const preset = providerCatalog.find(item => item.value === next);
    onConfigurationChange();
    setProvider({ ...provider, provider: next, model: preset?.defaultModel ?? provider.model, baseUrl: next === "openai_compatible" ? provider.baseUrl : "", apiKey: "" });
  };
  ...
  const testMessage = testing
    ...
    : testIsCurrent
      ...
      : testState === "failed"
        ? "O teste falhou. Corrija o item indicado e repita a validação."
        : requiresNewKey
  ...
  const updateTextModel = (model: string) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="agency-dialog agency-provider-dialog" showCloseButton={!saving} onPointerDownOutside={event => { if (saving) event.preventDefault(); }}>
        <form onSubmit={onSubmit}>
          <div className="agency-provider-dialog-header">
            <DialogHeader>
              <div className="agency-provider-dialog-title"><div><p className="ops-section-kicker">{editing ? "Editar conexão" : "Novo motor de IA"}</p><DialogTitle>{editing ? "Atualize o motor com segurança" : "Configure um motor para este cliente"}</DialogTitle></div><span className="agency-provider-dialog-icon"><KeyRound size={19} /></span></div>
              <DialogDescription>{editing ? "Revise os dados e valide novamente somente se mudar provedor, URL, modelo de texto ou chave." : "Escolha o provedor, informe apenas o necessário e valide a conexão antes de protegê-la."}</DialogDescription>
            </DialogHeader>
            <ol className="agency-provider-dialog-steps" aria-label="Etapas de configuração"><li className="is-current"><b>1</b><span>Motor</span></li><li className="is-current"><b>2</b><span>Configurar</span></li><li className={testIsCurrent ? "is-complete" : ""}><b>{testIsCurrent ? <CheckCircle2 size={12} /> : "3"}</b><span>Validar</span></li></ol>
          </div>
          <div className="agency-provider-dialog-body">
            ...
            <section className="agency-provider-configuration">
              ...
              <div className="agency-dialog-fields">
                <Field label="Nome da conexão" value={provider.label} onChange={label => update({ label })} placeholder="Ex.: NVIDIA · Conteúdo" required />
                {provider.provider === "nvidia" ? <section className="agency-nvidia-preset-picker" aria-labelledby="nvidia-model-picker-title"><div className="agency-nvidia-preset-heading"><div><span>Modelos NVIDIA NIM</span><strong id="nvidia-model-picker-title">Escolha um preset para responder</strong></div><small>Disponibilidade confirmada no teste.</small></div><div className="agency-nvidia-preset-grid" role="radiogroup" aria-label="Presets de modelos NVIDIA">{modelPresets.map((item, index) => <button key={item.value} className={provider.model === item.value ? "is-selected" : ""} type="button" role="radio" aria-checked={provider.model === item.value} onClick={() => updateTextModel(item.value)}><span>{index === 0 ? "Recomendado" : "Preset"}{provider.model === item.value ? <CheckCircle2 size={14} /> : null}</span><strong>{item.label}</strong><code>{item.value}</code></button>)}<button className={usesCustomModel ? "is-selected" : ""} type="button" role="radio" aria-checked={usesCustomModel} onClick={() => updateTextModel("")}><span>Personalizado{usesCustomModel ? <CheckCircle2 size={14} /> : null}</span><strong>Informar outro modelo</strong><small>Use o identificador habilitado para este cliente.</small></button></div><p>Os presets são sugestões. O catálogo e as permissões dependem da conta NVIDIA do cliente.</p></section> : <label className="agency-field"><span>Modelo de IA que responderá</span><select value={usesCustomModel ? "__custom__" : provider.model} onChange={event => updateTextModel(event.target.value === "__custom__" ? "" : event.target.value)}><option value="" disabled>Selecione o modelo</option>{modelPresets.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}<option value="__custom__">Informar identificador personalizado…</option></select></label>}
                {usesCustomModel ? <Field label="Identificador do modelo personalizado" value={provider.model} onChange={updateTextModel} placeholder="Ex.: fornecedor/modelo-habilitado" required /> : null}
                <Field label="Modelo de imagem (opcional)" value={provider.imageModel} onChange={imageModel => update({ imageModel })} placeholder="Modelo de imagem, se houver" />
                {provider.provider !== "manus" ? <><Field label={provider.provider === "openai_compatible" ? "URL base do gateway" : provider.provider === "nvidia" ? "URL NVIDIA NIM (opcional)" : "URL base (opcional)"} value={provider.baseUrl} onChange={baseUrl => update({ baseUrl }, true)} placeholder={provider.provider === "nvidia" ? "https://integrate.api.nvidia.com/v1" : "https://..."} /><Field label={requiresNewKey ? "Nova chave de API (obrigatória)" : editing ? "Nova chave de API (opcional)" : "Chave de API"} value={provider.apiKey} onChange={apiKey => update({ apiKey }, true)} placeholder="Cole a chave do cliente" type="password" autoComplete="new-password" required={!editing || requiresNewKey} /></> : <div className="agency-provider-integrated-note"><ShieldCheck size={16} /><p>O Manus integrado usa a infraestrutura da VERTEX. Nenhuma chave externa será solicitada ou armazenada.</p></div>}
              </div>
            </section>
            <section className="agency-provider-validation">
              ...
              <div className={`agency-connection-test ${testing ? "is-testing" : testIsCurrent ? "is-passed" : testState === "failed" ? "is-failed" : ""}`}><button className="ops-outline-button" type="button" disabled={testing} onClick={onTest}>{testing ? <Loader2 size={15} /> : testIsCurrent ? <CheckCircle2 size={15} /> : <RefreshCw size={15} />}{testing ? "Validando…" : testIsCurrent ? "Conexão validada" : "Testar conexão"}</button><span aria-live="polite">{testMessage}</span></div>
              {testing ? <div className="agency-provider-test-progress" role="status" aria-live="polite"><span /><p><strong>Conferindo chave, endpoint e acesso ao modelo.</strong> A credencial permanece somente nesta validação servidor a servidor.</p></div> : null}
              {showDiagnostic ? <div className="agency-provider-test-diagnostic" role="alert"><XCircle size={17} /><div><strong>{diagnostic.title}</strong><p>{diagnostic.detail}</p><small>{diagnostic.action}</small></div></div> : null}
            </section>
            <p className="agency-dialog-security"><ShieldCheck size={15} />A chave é usada somente no teste servidor a servidor, é cifrada ao salvar e jamais retorna à interface. {description}</p>
          </div>
          <DialogFooter className="agency-provider-dialog-footer"><button className="ops-text-button" type="button" disabled={saving} onClick={onCancel}>Cancelar</button><button className="ops-primary-button" type="submit" disabled={saving || testing || (requiresTest && !testIsCurrent)}>{saving ? <Loader2 size={16} /> : <ShieldCheck size={16} />}{editing ? "Salvar alterações" : "Proteger conexão"}</button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );

  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="agency-dialog agency-provider-dialog" showCloseButton={!saving} onPointerDownOutside={event => { if (saving) event.preventDefault(); }}><form onSubmit={onSubmit}><DialogHeader><div className="agency-provider-dialog-title"><div><p className="ops-section-kicker">{editing ? "Editar conexão" : "Novo motor de IA"}</p><DialogTitle>{editing ? "Atualize o motor com segurança" : "Configure um motor para este cliente"}</DialogTitle></div><span className="agency-provider-dialog-icon"><KeyRound size={19} /></span></div><DialogDescription>{editing ? "Revise a configuração antes de salvar. Alterações de motor, URL ou modelo exigem uma nova chave validada." : "Escolha o provedor, informe apenas o necessário e valide a conexão antes de protegê-la."}</DialogDescription></DialogHeader><ol className="agency-provider-dialog-steps" aria-label="Etapas de configuração"><li className="is-current"><b>1</b><span>Motor</span></li><li className="is-current"><b>2</b><span>Configurar</span></li><li className={testIsCurrent ? "is-complete" : ""}><b>{testIsCurrent ? <CheckCircle2 size={12} /> : "3"}</b><span>Validar</span></li></ol><section className="agency-provider-choice"><div className="agency-provider-section-heading"><div><span>Etapa 1</span><strong>Escolha o motor</strong></div><small>Selecione a infraestrutura que atenderá este cliente.</small></div><div className="agency-provider-option-grid" role="radiogroup" aria-label="Motor de IA">{providerCatalog.map(item => <button key={item.value} type="button" className={provider.provider === item.value ? "is-selected" : ""} role="radio" aria-checked={provider.provider === item.value} onClick={() => chooseProvider(item.value)}><ProviderBrandIcon provider={item.value} /><strong>{item.label}</strong><small>{item.value === "manus" ? "Sem chave externa" : item.value === "openai_compatible" ? "Gateway compatível" : "Chave do cliente"}</small></button>)}</div><div className="agency-provider-selected-card"><span className="agency-provider-selected-icon"><ProviderBrandIcon provider={selectedProvider.value} /></span><div><span>Motor selecionado</span><strong>{selectedProvider.label} · {selectedProvider.defaultModel}</strong><p>{selectedProvider.description}</p></div><em>{selectedProvider.value === "manus" ? "Sem chave" : "Chave própria"}</em></div>{selectedProvider.docsUrl ? <a className="agency-provider-doc-link" href={selectedProvider.docsUrl} target="_blank" rel="noreferrer"><ExternalLink size={13} /> {selectedProvider.docsLabel} <span>abre em nova aba</span></a> : <p className="agency-provider-doc-note">{selectedProvider.value === "openai_compatible" ? "Use a documentação do gateway contratado para confirmar URL, modelo e permissões." : "A VERTEX gerencia este motor integrado sem exigir documentação ou chave externa."}</p>}</section><section className="agency-provider-configuration"><div className="agency-provider-section-heading"><div><span>Etapa 2</span><strong>Defina a configuração</strong></div><small>{provider.provider === "manus" ? "O motor integrado dispensa chave externa." : "Use os dados autorizados pelo cliente."}</small></div><div className="agency-dialog-fields"><Field label="Nome da conexão" value={provider.label} onChange={label => update({ label })} placeholder="Ex.: OpenAI · Marketing" required /><Field label="Modelo de texto" value={provider.model} onChange={model => update({ model }, true)} placeholder="Modelo de texto" required /><Field label="Modelo de imagem (opcional)" value={provider.imageModel} onChange={imageModel => update({ imageModel }, true)} placeholder="Modelo de imagem, se houver" />{provider.provider !== "manus" ? <><Field label={provider.provider === "openai_compatible" ? "URL base do gateway" : "URL base (opcional)"} value={provider.baseUrl} onChange={baseUrl => update({ baseUrl }, true)} placeholder="https://..." /><Field label={requiresNewKey ? "Nova chave de API (obrigatória)" : editing ? "Nova chave de API (opcional)" : "Chave de API"} value={provider.apiKey} onChange={apiKey => update({ apiKey }, true)} placeholder="Cole a chave do cliente" type="password" autoComplete="new-password" required={!editing || requiresNewKey} /></> : <div className="agency-provider-integrated-note"><ShieldCheck size={16} /><p>O Manus integrado usa a infraestrutura da VERTEX. Nenhuma chave externa será solicitada ou armazenada.</p></div>}</div></section><section className="agency-provider-validation"><div className="agency-provider-section-heading"><div><span>Etapa 3</span><strong>Teste antes de proteger</strong></div><small>O salvamento fica bloqueado enquanto a validação obrigatória não terminar.</small></div><div className={`agency-connection-test ${testing ? "is-testing" : testIsCurrent ? "is-passed" : testState === "failed" ? "is-failed" : ""}`}><button className="ops-outline-button" type="button" disabled={testing} onClick={onTest}>{testing ? <Loader2 size={15} /> : testIsCurrent ? <CheckCircle2 size={15} /> : <RefreshCw size={15} />}{testing ? "Validando…" : testIsCurrent ? "Conexão validada" : "Testar conexão"}</button><span aria-live="polite">{testMessage}</span></div>{testing ? <div className="agency-provider-test-progress" role="status" aria-live="polite"><span /><p><strong>Conferindo chave, endpoint e acesso ao modelo.</strong> A credencial permanece somente nesta validação servidor a servidor.</p></div> : null}{diagnostic ? <div className="agency-provider-test-diagnostic" role="alert"><XCircle size={17} /><div><strong>{diagnostic.title}</strong><p>{diagnostic.detail}</p><small>{diagnostic.action}</small></div></div> : null}</section><p className="agency-dialog-security"><ShieldCheck size={15} />A chave é usada somente no teste servidor a servidor, é cifrada ao salvar e jamais retorna à interface. {description}</p><DialogFooter><button className="ops-text-button" type="button" disabled={saving} onClick={onCancel}>Cancelar</button><button className="ops-primary-button" type="submit" disabled={saving || testing || (requiresTest && !testIsCurrent)}>{saving ? <Loader2 size={16} /> : <ShieldCheck size={16} />}{editing ? "Salvar alterações" : "Proteger conexão"}</button></DialogFooter></form></DialogContent></Dialog>;
}
